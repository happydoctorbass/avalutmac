...
export function AdminPanel({ currentCards = [], onCardsGenerated }: AdminPanelProps) {
  ...
  const startGame = () => {
    ...
    // Fallback to random if manual is incomplete or not applicable
    const validManual = manualCards.slice(0, cardCount).every(c => c !== null);
    ...
    sendCommand(GAME_EVENTS.TOGGLE_STATE, {
      state: 'GAME', cards, playerId: playerId.trim(), betAmount: Number(betAmount), language, gameType, cardCount, timerDuration: Number(timerDuration) || 0
    });
  };

  const handleRevealAll = () => sendCommand(GAME_EVENTS.REVEAL_ALL, {});
        ...
        <div className={styles.rightCol}>
          <section className={styles.block}>
            <div className="flex justify-between items-center mb-4">
              <h2 className={styles.blockTitle} style={{ marginBottom: 0 }}>Управление картами</h2>
              <button onClick={handleRevealAll} className={ui.btnSecondary} style={{ width: 'auto', padding: '0.25rem 0.75rem' }}>ОТКРЫТЬ ВСЕ</button>
            </div>
            ...
            <AdminCardGrid 
              ...
              revealedStates={currentCards.length > 0 ? currentCards.map((_, i) => false) : Array(5).fill(false)} // This is a simplification for visual state.
            />
          </section>
        </div>
      </div>
